import type { I18nextError } from '../types'
...
export class I18nextErrorHandler {
  ...
  static transformError(error: any, url: string): I18nextError {
    ...
    if (error?.message) {
      ...
      const networkErrorTerms = ['failed', 'fetch', 'network', 'load', 'timeout', 'abort']

      const isNetworkError = networkErrorTerms.some(term => errorMessage.includes(term))

...
import type { I18nextError, I18nextRequestOptions, I18nextResponse, RequestCallback } from '../types'
import { I18nextErrorHandler } from './error-handling'
...
export class TanStackI18nextBackend {
  ...
  private buildFetchOptions(payload?: any): RequestInit {
    ...
    // Add custom headers
    const customHeaders =
      typeof this.options.customHeaders === 'function' ? this.options.customHeaders() : this.options.customHeaders
    ...
    return {
      ...
      body: payload ? (this.options.stringify || JSON.stringify)(payload) : undefined,
  ...
  private createQueryFn = (url: string, payload?: any) => {
    return async (): Promise<I18nextResponse> => {
      try {
        ...
        const finalUrl = this.addQueryString(url, this.options.queryStringParams)
        ...
        if (!response.ok) {
          const error = new Error(`HTTP ${response.status}: ${response.statusText}`) as I18nextError
        ...
        try {
          ...
          const error = new Error(`Failed to parse response from ${url}`) as I18nextError
  ...
  async requestWithCallback(
    ...
    try {
      ...
      const processedError = I18nextErrorHandler.processForCallback(error as I18nextError)
      callback(processedError.error, processedError.retry ? ({ status: 0 } as I18nextResponse) : null)
    }
  }
   ...
   */
  async request(options: I18nextRequestOptions, url: string, payload?: any): Promise<I18nextResponse> {
    // Merge options for this request
    const mergedOptions = { ...this.options, ...options }

    // Create query key
    const queryKey = payload ? queryKeys.create(url, payload) : queryKeys.load(url)
    ...
    const queryOptions = {
      ...
      retry: (failureCount: number, error: I18nextError) => {
        const maxRetries =
          typeof mergedOptions.tanstackQuery?.retry === 'number' ? mergedOptions.tanstackQuery.retry : 3

        if (failureCount >= maxRetries) return false
        return I18nextErrorHandler.shouldRetry(error)
      },
      retryDelay: mergedOptions.tanstackQuery?.retryDelay || I18nextErrorHandler.getRetryDelay,
      refetchOnWindowFocus: mergedOptions.tanstackQuery?.refetchOnWindowFocus ?? false,
      refetchOnReconnect: mergedOptions.tanstackQuery?.refetchOnReconnect ?? true
    }

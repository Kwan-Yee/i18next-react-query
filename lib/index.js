...
class Backend {
  ...
  init(services, options = {}, allOptions = {}) {
    ...
    // Initialize TanStack Query backend if available and enabled
  ...
  async initTanStackBackend() {
    // Initialize TanStack Query backend if enabled and dependencies are available
    if (!this.options.tanstackQuery?.enabled || !this.options.queryClient) {
      this.tanstackBackend = null
      return
    }

    try {
      // Dynamically import TanStack backend to avoid hard dependency
      const { TanStackI18nextBackend } = await import('./tanstack/tanstack-request.js')
      this.tanstackBackend = new TanStackI18nextBackend(this.options.queryClient, this.options)
    } catch (error) {
      // Gracefully handle missing TanStack Query dependencies
      if (this.services?.logger) {
        this.services.logger.warn(
          'TanStack Query dependencies not found. Falling back to standard HTTP backend.',
          error.message
        )
      }
      this.tanstackBackend = null
  ...
  loadUrl(url, callback, languages, namespaces) {
    ...
    // Use TanStack Query backend if available
    if (this.tanstackBackend) {
      try {
        this.tanstackBackend.requestWithCallback(this.options, url, payload, (err, res) => {
        ...
        // If TanStack backend fails, fall back to standard request
        if (this.services?.logger) {
          this.services.logger.warn('TanStack Query backend failed, falling back to standard request:', error.message)
        }
        // Continue to fallback implementation below
      }
    }

    // Fallback to original request implementation
    this.options.request(this.options, url, payload, (err, res) => {
      ...
      if (!res && err && err.message) {
        ...
        // Safari: "Load failed"
        const isNetworkError = ['failed', 'fetch', 'network', 'load'].find(term => errorMessage.indexOf(term) > -1)
        if (isNetworkError) {
  ...
  create(languages, namespace, key, fallbackValue, callback) {
    ...
    const makeRequest = lng => {
      ...
      if (this.tanstackBackend) {
        try {
          this.tanstackBackend.requestWithCallback(this.options, url, payload, (data, res) => {
          ...
          if (this.services?.logger) {
            this.services.logger.warn(
              'TanStack Query backend failed for create operation, falling back to standard request:',
              error.message
            )
          }
          // Fallback to original request implementation
